...
for (i in unique(cexall$kidcat)) {
  cexall <- cexall %>%
    mutate(!!paste0("kidd", i) := as.numeric(kidcat == i))
}
```
...
finprice <- read_dta("finprice.dta")
...
psid <- psid %>%
  mutate(y2 = y^2, y3 = y^3, y4 = y^4, y5 = y^5, ncomp2 = ncomp^2, post86 = year > 1986)

lm_tax <- lm(ftax ~ y + y2 + y3 + y4 + y5 + ncomp + ncomp2 + post86 + kids + self + asset, data = psid %>% filter(year < 1991))
psid <- psid %>%
