...
ggplot(long, aes(x = time, y = mean, color = variable)) + 
  ...
  labs(x = "Year", 
       ...
       color = "Model") +
  scale_color_manual(
    values = c("gbm_ndur" = "#F28C8C", "lr_ndur" = "#A8E1A1", "paper_ndur" = "#A8C9F0"),  # Assign colors to each line
    labels = c("Gradient Boosting Machines", "Multiple Linear Regression", "Inversion Model")  # Rename legend items
  ) +
  scale_x_continuous(
    breaks = seq(min(long$time), max(long$time), by = 1)  # Set breaks to every year
  ) +
  theme_minimal() +
...
Plot results of Monte Carlo simulations:
```{r}
...
# Assuming mc_results contains columns 'Model' and 'RMSE'

# Apply Shapiro-Wilk test for each model
shapiro_results <- mc_results %>%
  group_by(Model) %>%
  summarise(
    "p-value" = shapiro.test(RMSE)$p.value  # Apply the Shapiro-Wilk test
  )
