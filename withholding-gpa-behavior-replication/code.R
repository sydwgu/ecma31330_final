### DETAILS ====================================================================
# Code to produce figures and tables from Floyd, Lee, and Tomar (2023). Figures
# 1-4 and Tables 1-6 use confidential data; thus, we instead provide synthetic
# datasets to illustrate the data structure and show that the code compiles.
# Thus, the results produced will not match the paper. Tables 7-9 are based on
# publicly available employment profiles as described in the paper. The data
# provided are the actual data. Thus the results produced should match those in
# the paper. “educ” and “emp” above are derived from four primary tables (we
# consider these proprietary from a research perspective) using an R code block
# provided in a comment on line 671 To find the code that generates a
# particular figure or  table, search the code for that figure or table (e.g.,
# search "FIGURE 3" or "TABLE 7").
...
legitN <- function(dataInput) {
  return(!is.na(dataInput) & !is.infinite(dataInput) & !is.nan(dataInput));
}

renamer <- function(dataInput, separator) {
  simpleCap <- function(stringInput, direction) {
    s <- strsplit(stringInput, " ")[[1]];
    if (direction== "upper") {return(paste(toupper(substring(s, 1,1)),
                                       substring(s, 2), sep="", collapse=" "));
    }
    if (direction== "lower") {return(paste(tolower(substring(s, 1,1)),
                                       substring(s, 2), sep="", collapse=" "));
    }}}

Mean <- function(x) signif(base::mean(x, na.rm=TRUE), 3);
SD <- function(x) signif(stats::sd(x, na.rm=TRUE), 3);
q5 <- function(x) signif(stats::quantile(x, 0.01, na.rm=TRUE), 2);
q25 <- function(x) signif(stats::quantile(x, 0.25, na.rm=TRUE), 2);
q50 <- function(x) signif(stats::quantile(x, 0.50, na.rm=TRUE), 2);
q75 <- function(x) signif(stats::quantile(x, 0.75, na.rm=TRUE), 2);
q95 <- function(x) signif(stats::quantile(x, 0.99, na.rm=TRUE), 2);
...
gra <- gra[
    ...
    , id
      ...
      .(max(ifelse(1:.N == 1, year, NA), na.rm= T),
        max(ifelse(1:.N == 1, term, NA), na.rm= T))
    , id];


### CLEAN JOB OFFERS ===========================================================
jof <- jof[
  ][decisionDate!= as.Date("1900-01-01") & !is.na(id) &
      jobOfferStatus== "Accepted"
  ][, c("fixedSalary", "negFixed"):=
      .(sum(baseSalary, earlySignonBonusAmt, signingstartingBonusAmt,
            gauranteedYeaarendBonusAmt, tuitionReimbursementBonusAmt,
            relocationExpenseBonusAmt, na.rm= T),
        (baseSalary < 0 & !is.na(baseSalary)) |
          (earlySignonBonusAmt< 0 & !is.na(earlySignonBonusAmt)) |
          (signingstartingBonusAmt< 0 & !is.na(signingstartingBonusAmt)) |
          (gauranteedYeaarendBonusAmt< 0 & !is.na(gauranteedYeaarendBonusAmt)) |
          (tuitionReimbursementBonusAmt< 0 & !is.na(tuitionReimbursementBonusAmt)) |
          (relocationExpenseBonusAmt< 0 & !is.na(relocationExpenseBonusAmt)))
    , .(id, surveyInstance)];

jofDups <- jof[
  ][duplicated(jof[, .(organizationName, positionType, id)]) |
      duplicated(jof[, .(organizationName, positionType, id)], fromLast = T)
  ][, .SD[fixedSalary== max(fixedSalary)], id
  ][, .SD[decisionDate== max(decisionDate)], id
  ][, .SD[offerAnswerDate== max(offerAnswerDate)], id
  ][, .SD[surveyInstance== max(surveyInstance)], id];
  # iteratively filter out duplicates

jof <- rbind(jof, jofDups);

jof <- jof[
  ][, c("nPositions", "nEmployers"):=
      .(length(organizationName),
        length(unique(organizationName)))
    , id
  ][order(nPositions, id, decisionDate)
  ][, priorAccept:= positionType== "Permanent" & nPositions > nEmployers
  ][, .(id, organizationName, positionType, priorAccept,
        fixedSalary, negFixed, jobOfferStatus)];
  # nPositions > nEmployers when a student did an internship with an employer
  # from whom they eventually accepted a permanent position


### CLEAN GMATs ================================================================
ts <- ts[
  ][testCategory== "gmat_total"
  ][, .SD[gmat== max(gmat)]
    , id];
...
### CLEAN JOB RANKINGS =========================================================
jra[, employer:= enc2native(employer)];
jra[grepl("Nestl", employer), employer:= "Nestle"];

jra <- rbind(jra[year==2000][, year:= 1997], jra[year==2000][, year:= 1998],
             jra[year==2000][, year:= 1999], jra);
  # Universum rankings not available before 2000, so extrapolate backwards


### MERGE DATA AND TIDY UP =====================================================                           
labor <- merge(gra, jof, "id", allow.cartesian= T);

labor <- labor[
  ][, numYears:= max(year) - min(year)
    , id
  ][(positionType== "Internship" & term== 4 & year== firstYr) |
    (positionType== "Permanent" & priorAccept== T & term== 4 & year== firstYr) |
    (positionType== "Permanent" & priorAccept== F & term== 3 & year== firstYr + 1)];
  # For internship positions and permanent positions where a prior internship
  # was completed, select GPA at end of first fall. For permanent positions,
  # select GPA at end of first spring.

labor <- merge(labor, ts, "id");

strip1 <- " Group| Company| Partners| Incorporated| Corporation| Inc| Inc\\.| & Company| & Co\\.| Co\\.| GmbH| Ltd\\.|"
strip2 <- " LLC| L\\.P\\.| Corp\\.| Plc| LLP| L\\.L\\.C\\.|\\.|,| Capital| Management| North America| Ltd| Consulting|"
strip3 <- " Financial Services|\\*| Corp| International| Financial|"
strip4 <- " Technologies|The | Investments| Securities| Holdings| Roebuck| HPC| Brands| Scientific| united states|"
strip5 <- " tohmatsu| benson| global| S\\.A\\.| ag| & touche";

toStrip <- paste0(strip1, strip2, strip3, strip4, strip5);

Encoding(jra$employer) <- 'latin1';
labor[, organizationNameMerge:= tolower(gsub(toStrip, "", organizationName, T))];
jra[, employerMerge:= tolower(gsub(toStrip, "", employer, T))];
  # Make the school data and job rank data organization names similar

matches <- data.frame(namesData= as.character(), namesJra= as.character(),
                      year= as.numeric());

for (i in 1998:2007) {
  miniJra <- jra[year== i];
  miniData <- labor[year== i];
  
  organizationNameMerge <- unique(miniData$organizationNameMerge);
  organizationNameMerge <- organizationNameMerge[order(organizationNameMerge)];
  
  employerMerge <- unique(miniJra$employerMerge);
  employerMerge <- employerMerge[order(employerMerge)];
  
  sd <- stringdistmatrix(employerMerge, organizationNameMerge);
  
  matchQuality <- 1 - apply(sd, 2, min)*1/nchar(organizationNameMerge);
  
  matchesMini <- cbind(organizationNameMerge, 
                 employerMerge= employerMerge[apply(sd, 2, which.min)], year= i,
                 matchQuality);
  matches <- rbind(matches, matchesMini[matchQuality >= 0.8, c(1:3)]);
}
  # 0.8 always leads to a good match (hand-match the rest below)

matches <- data.table(matches)[, year:= as.numeric(year)];
  #--- Initial matches using string distance

matches[organizationNameMerge %in% c("ubs investment bank"), organizationNameMerge:= "ubs"];
matches <- matches[!grepl("deloitte", organizationNameMerge, T)];

jra <- jra[!(employer=="KPMG" & year== 2002) &
               !(grepl("Boston Scientific", employer) & year== 2004) &
               !(grepl("Boston Scientific", employer) & year== 2006)];
  # Ambiguous entries

matches <- rbind(matches,
  list("kraft foods", "kraft", 2001),
  list("ubs", "warburg dillon read/ubs", 1998),
  list("ubs", "warburg dillon read/ubs", 1999),
  list("ubs", "warburg dillon read/ubs", 2000),
  list("ubs", "ubs/warburg dillon read", 2001),
  list("ubs", "ubs/warburg", 2002),
  list("ubs", "ubs/warburg", 2003),
  list("ubs", "ubs", 2007),
  list("boston", "boston consutling", 2002),
  list("siebel systems", "siebel", 1999),
  list("siebel systems", "siebel", 2000),
  list("siebel systems", "siebel", 2001),
  list("putnam investment", "putnam", 1998),
  list("putnam investment", "putnam", 1999),
  list("putnam investment", "putnam", 2000),
